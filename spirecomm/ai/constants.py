...
MAX_MONSTER_COUNT = 10 # 最多同时存在的怪物数量：蛇女+四个匕首 或者 五只小史莱姆。有待求证  -> 还真不是 死了的怪物还是会占据位置。is_gone=True. 所以可能取决于召唤的小怪的数量。
...
MAX_SCREEN_ITEMS = 120 # 屏幕上可见物品的最大数量,以升级时卡组可能的最大值为参考
MAX_SCREEN_MISC_DIM = 8 # 屏幕misc特征的维度
MAX_SCREEN_ITEM_FEAT_DIM = 16 # 屏幕物品特征的维度
MAX_REST_OPTIONS = 6 # 休息处选项
MAX_CHOOSE_COUNT = 100 # 假设可以选择100张牌，主要是在升级的时候
MAX_CHOICE_LIST = 100 # 最大编码的 choice 数量（可调整）
MAX_VOCAB_SIZE = 5000 # 词汇表大小